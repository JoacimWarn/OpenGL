...
void drawEnemy(){
	...
	glVertex3f(-13.0f, enemyLoc.y, 0.0f);
	glVertex3f(-13.0f + 0.5f, enemyLoc.y, 0.0f);
	glVertex3f(-13.0f + 0.5f, enemyLoc.y - 0.5f, 0.0f);
	glVertex3f(-13.0f, enemyLoc.y - 0.5f, 0.0f);
...
void drawProjectile(){
	...
	static float dx[BUFFER] = {-13.0f, -11.0f, -9.0f, -7.0f, -5.0f};
	...
	for(int i = 0; i < BUFFER; i++){
		...
		if(dx[i] >= 14.0f){			//when a new laser will spawn
...
void enemyMoves(){
	...
	if(enemyLoc.y > (UPPER_LIMIT - 2.5f)) dy = - dy;
	if(enemyLoc.y < (LOWER_LIMIT + 2.5f)) dy = - dy;
	
	if(glfwGetTime() < 0.1f){
		
		enemyLoc.y = enemyLoc.y - dy;
		glfwSetTime(0.0f);
...
int main(){
	...
	while(!glfwWindowShouldClose(window) && !playerHit){
		...
		enemyMoves();	//enemy follows players y position
